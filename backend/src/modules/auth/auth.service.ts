    ...
    return this.prisma.$transaction(async (tx) => {
      ...
      await tx.user.create({
        data: {
          ...
          passwordHash, role: UserRole.SUPER_ADMIN, tenantId: tenant.id, isActive: true,
        },
      });
      ...
      await tx.user.create({
        data: {
          ...
          passwordHash, role: UserRole.TENANT_ADMIN, tenantId: tenant.id, isActive: true,
        },
      });
      ...
      await tx.user.create({
        data: {
          ...
          passwordHash, role: UserRole.DOCTOR, tenantId: tenant.id, isActive: true,
        },
      });
