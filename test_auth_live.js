...
function post(path, body, cookieStr, csrfToken) {
  return new Promise((resolve) => {
    ...
    const opts = {
      host: 'femcare-backend-api.onrender.com', port: 443, path, method: 'POST',
      headers,
      timeout: 15000
    };
...
function get(path, cookieStr) {
  return new Promise((resolve) => {
    const opts = {
      host: 'femcare-backend-api.onrender.com', port: 443, path,
...
async function main() {
  console.log('=== LIVE AUTH FLOW TEST ===\n');

  // 1. Login
  console.log('1. POST /api/auth/login');
  const login = await post('/api/auth/login', { email: 'admin@gmail.com', password: '123456' });
